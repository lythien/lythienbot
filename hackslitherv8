...
// ==UserScript==
// @name         Hack game Slither V8.3
// @namespace    http://slither.io/
// @version      0.8.3 -  LyThien.Top
...
var userInterface = (function() {
    ...
    return {
        ...
        onFrameUpdate: function() {
            ...
            window.botstatus_overlay.innerHTML = generalStyle + '(T / Chuột Phải) Bật Tắt Hack: </span>' + userInterface.handleTextColor(bot.isBotRunning);
            window.visualdebugging_overlay.innerHTML = generalStyle + '(Y) Hiển thị chế độ hack: </span>' + userInterface.handleTextColor(window.visualDebugging);
            window.autorespawn_overlay.innerHTML = generalStyle + '(I) Tự động đăng nhập: </span>' + userInterface.handleTextColor(window.autoRespawn);
            window.rotateskin_overlay.innerHTML = generalStyle + '(W) Tự động thay đổi Skin: </span>' + userInterface.handleTextColor(window.rotateskin);
            window.rendermode_overlay.innerHTML = generalStyle + '(O) Giao diện điện thoại: </span>' + userInterface.handleTextColor(window.mobileRender);
            window.huntprey_overlay.innerHTML = generalStyle + '(P) Săn Mồi: </span>' + userInterface.handleTextColor(window.huntPrey);
            window.collision_detection_overlay.innerHTML = generalStyle + '(C) Phát hiện né va chạm: </span>' + userInterface.handleTextColor(window.collisionDetection);
            window.head_collision_radius_multiplier_overlay.innerHTML = generalStyle + '(A/S) Tăng giảm bán kính: ' + window.headCollisionRadiusMultiplier + ' </span>';
            window.body_collision_radius_multiplier_overlay.innerHTML = generalStyle + '(E/R) Tăng giảm bán kính thân: ' + window.bodyCollisionRadiusMultiplier + ' </span>';
            window.defence_overlay.innerHTML = generalStyle + '(D) Xoay Vòng Tròn: </span>' + userInterface.handleTextColor(window.defence);
            window.resetzoom_overlay.innerHTML = generalStyle + '(Z) Bản Đồ Mặc Định </span>';
            window.scroll_overlay.innerHTML = generalStyle + '(Lăn Chuột) Phóng To Thu Nhỏ Bản Đồ </span>';
            window.quittomenu_overlay.innerHTML = generalStyle + '<span style=\"color:' + ('yellow;\">-- Xem Thêm : LyThien.Top --') + '</span></span>';
            window.changeskin_overlay.innerHTML = generalStyle + '(X) Đổi Skin </span>';
            window.clusteringchange_overlay.innerHTML = generalStyle + '(F) Thay Đổi Thuật Toán: ' + (bot.foodClusteringMode == 0 ? 'chancity' : 'ksofiyuk') + '</span>';
            window.quickResp_overlay.innerHTML = generalStyle + '(ESC) Chơi Lại </span>';
            window.logdebugging_overlay.innerHTML = generalStyle + '(U) Hiển Thị Lổi </span>' + userInterface.handleTextColor(window.logDebugging);
        ...
        handleTextColor: function(enabled) {
            return '<span style=\"opacity: 0.8; color:' + (enabled ? 'green;\">kích hoạt' : 'red;\">tắt') + '</span>';
        }
    };
})();
...
(function() {
    ...
    window.nick.value = userInterface.loadPreference('savedNick', 'LyThien.Top-Bot');
